...
describe("prepareMascotDisplayAsset", () => {
  ...
  it("remove o quadriculado escuro usado por imagens sem transparência real", async () => {
    const background = await darkCheckerboard(160, 160);
    const source = await sharp(background, { raw: { width: 160, height: 160, channels: 4 } })
      .composite([{ input: Buffer.from('<svg width="72" height="104" xmlns="http://www.w3.org/2000/svg"><ellipse cx="36" cy="52" rx="30" ry="46" fill="#c91f37" stroke="#171310" stroke-width="6"/></svg>'), left: 44, top: 28 }])
      .png()
      .toBuffer();
    ...
    const decoded = await sharp(result.bytes).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
    expect(decoded.data[3]).toBe(0);
    const center = ((Math.floor(decoded.info.height / 2) * decoded.info.width + Math.floor(decoded.info.width / 2)) * 4) + 3;
    expect(decoded.data[center]).toBe(255);
  });
...
function darkCheckerboard(width: number, height: number) {
  const data = Buffer.alloc(width * height * 4);
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      const shade = (Math.floor(x / 8) + Math.floor(y / 8)) % 2 === 0 ? 33 : 57;
      const offset = (y * width + x) * 4;
      data[offset] = shade;
      data[offset + 1] = shade;
      data[offset + 2] = shade;
