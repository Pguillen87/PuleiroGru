...
describe("jobs owner-scoped e retomáveis", () => {
  it("reaproveita o mesmo job para owner e attemptId", async () => {
    ...
    const input = { ...image, ownerId: "owner-a", attemptId: crypto.randomUUID(), correlationId: crypto.randomUUID(), idempotencyKey: "register-1" };
  ...
  it("não permite descoberta por outro owner", async () => {
    const provider = new MockMascotGenerationProvider();
    const input = { ...image, ownerId: "owner-a", attemptId: crypto.randomUUID(), correlationId: crypto.randomUUID(), idempotencyKey: "register" };
