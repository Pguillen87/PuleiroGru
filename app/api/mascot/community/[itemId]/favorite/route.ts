...
export async function POST(request: Request, context: { params: Promise<{ itemId: string }> }) {
  try {
    ...
    if (identity.mode !== "supabase-session") return NextResponse.json({ message: "Entre para favoritar." }, { status: 401 });
    const client = createAdminClient();
    if (!client) return NextResponse.json({ message: "A comunidade ainda não está configurada neste ambiente." }, { status: 503 });
    await setCommunityRelation(client, "mascot_public_mascot_favorites", identity.uid, itemId, body.enabled);
  ...
  } catch (error) { return authErrorResponse(error) ?? NextResponse.json({ message: "Não foi possível atualizar o favorito." }, { status: 500 }); }
}
