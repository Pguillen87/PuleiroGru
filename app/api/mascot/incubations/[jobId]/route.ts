...
import { findAttemptByJobId, incubationProductState, saveAttemptJob } from "@/lib/mascot-generation/attempt-store";
...
export async function GET(request: Request, { params }: { params: Promise<{ jobId: string }> }) {
  try {
    ...
    return NextResponse.json({ job: { ...job, productState: attempt.hatched_at ? "HATCHED" : job.productState ?? incubationProductState(attempt), hatchedAt: attempt.hatched_at ?? undefined } });
  } catch (error) {
